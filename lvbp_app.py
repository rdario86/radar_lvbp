...
st.set_page_config(page_title="Predicción LVBP Automatizada", layout="wide", page_icon="⚾")

st.title("⚾ Predicción LVBP: Radar Diario Automatizado")
st.markdown("Proyección Sabermétrica adaptada a la Liga de Invierno")
st.markdown("---")

MAX_DEPTH_ELO = 5                

# 1. WHITELIST DE LA LVBP
LVBP_TEAM_WHITELIST = [
    "Leones del Caracas", "Navegantes del Magallanes", "Tiburones de La Guaira", 
    "Cardenales de Lara", "Aguilas del Zulia", "Tigres de Aragua", 
    "Caribes de Anzoategui", "Bravos de Margarita",
    # Variaciones de capitalización que la API a veces escupe
    "Leones Del Caracas", "Navegantes Del Magallanes", "Tiburones De La Guaira",
...
def get_hit_hunters(anio, fecha_hoy):
    try:
        # 2. ADAPTACIÓN A SPORTID=17
        juegos_hoy = statsapi.schedule(date=fecha_hoy, sportId=17)
        equipos_hoy = {}
...
# Reutilizamos las variables en session_state (solo cambiamos visualmente el nombre a LVBP para facilidad)
if 'df_lvbp' not in st.session_state: st.session_state.df_lvbp = None
...
st.sidebar.markdown("Las fechas cambian estrictamente a las 12:00 AM. Selecciona días pasados para auditar el rendimiento del radar en la LVBP.")

tz_et = 'America/Caracas' # Ajuste a huso horario venezolano
...
if st.sidebar.button("🔄 Descargar Historial Base", type="primary"):
    with st.spinner("Actualizando base de datos central de la LVBP..."):
        try:
            # 4. ADAPTACIÓN DEL CALENDARIO DE LVBP (OCTUBRE A ENERO)
            if datetime.datetime.now().month <= 2:
                anio_inicio = anio_sel - 1
            else:
                anio_inicio = anio_sel

            fechas = [
                (f"{anio_inicio}-10-01", f"{anio_inicio}-10-31"),
                (f"{anio_inicio}-11-01", f"{anio_inicio}-11-30"),
                (f"{anio_inicio}-12-01", f"{anio_inicio}-12-31"),
                (f"{anio_inicio+1}-01-01", f"{anio_inicio+1}-01-31")
            ]
            ...
            for start, end in fechas:
                ...
                while intentos < 3 and not exito:
                    try:
                        # 5. ADAPTACIÓN A SPORTID=17 PARA OBTENER TODO EL CARIBE
                        batch = statsapi.schedule(start_date=start, end_date=end, sportId=17)
            ...
            # 6. FILTRAMOS ESTRICTAMENTE LOS 8 EQUIPOS VENEZOLANOS
            df_full = df_full[df_full['Local'].isin(LVBP_TEAM_WHITELIST) & df_full['Visitante'].isin(LVBP_TEAM_WHITELIST)]
            
            # Reset de ELO solo para la LVBP
            elo_dict = {team: 1500.0 for team in LVBP_TEAM_WHITELIST}
            h_elo_l, h_elo_v = [], []
            ...
            st.session_state.df_lvbp = df_full
            st.session_state.modelo_ia = None
            st.sidebar.success("✅ Base de datos LVBP al día.")
        except Exception as e: st.sidebar.error(f"Error Crítico: {e}")

if st.session_state.df_lvbp is not None:
    df_historico = st.session_state.df_lvbp.copy()
    ...
    if len(df_filtrado) > 0 and necesita_entrenar:
        st.sidebar.info("🧠 Entrenando IA de la Liga Invernal...")
        df_filtrado = df_filtrado.sort_values('Date').reset_index(drop=True)
        
        x_racha_diff, x_h2h, x_luck_diff, x_split_diff, y_win = [], [], [], [], []
        # Reducimos un poco el tail de entrenamiento dado que las temporadas invernales son más cortas
        df_train = df_filtrado.tail(300).copy() 
        ...
        for idx, row in df_train.iterrows():
            ...
            if len(df_pasado) < 30: # Ajustado para inicio de temporada LVBP
        ...
        st.sidebar.success("✅ IA LVBP lista.")
    ...
    with tab1:
        st.markdown(f"### 🎯 Partidos LVBP programados para el: **{st.session_state.fecha_hoy}**")
        ...
        if st.button("⚡ Analizar y Evaluar Cartelera", type="primary", use_container_width=True):
            if len(df_filtrado) == 0:
                ...
                with st.spinner("Escaneando predicciones y verificando con resultados reales..."):
                    # 7. SPORTID=17 AQUÍ TAMBIÉN
                    juegos_hoy = statsapi.schedule(date=st.session_state.fecha_hoy, sportId=17)
                    ...
                    if not juegos_hoy:
                        ...
                        for juego in juegos_hoy:
                            ...
                            # 8. FILTRO WHITELIST LVBP
                            if e_local not in LVBP_TEAM_WHITELIST or e_visita not in LVBP_TEAM_WHITELIST: continue
                            ...
                            if game_dt_str:
                                try:
                                    ...
                                    hora_et = dt.tz_convert('America/Caracas').strftime('%I:%M %p')
                                except: hora_et = 'TBD'
                            else: hora_et = 'TBD'
                        ...
                        st.success("✅ Análisis LVBP completado.")
        ...
        if "resultados_jornada" in st.session_state and st.session_state.resultados_jornada is not None:
            ...
            st.download_button(
                label="📥 Descargar Cartelera LVBP (Excel)",
                data=excel_cartelera,
                file_name=f"cartelera_lvbp_{st.session_state.fecha_hoy}.xlsx",
    ...
    with tab2:
        ...
        if st.button("🎯 Cazar Bajas del Día (Top 4)", type="primary", use_container_width=True):
            with st.spinner("Haciendo el cruce de vulnerabilidad y auditando ponches finales..."):
                ...
                if resultados_k:
                    ...
                    st.warning("No hay suficientes datos de pitcheo para evaluar esta jornada en la LVBP.")
        ...
        if clave_k in st.session_state and st.session_state[clave_k] is not None:
            ...
            st.download_button(
                ...
                data=excel_ponches,
                file_name=f"caza_ponches_lvbp_{st.session_state.fecha_hoy}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            )
    ...
    with tab3:
        ...
        if st.button("🔎 Buscar Bateadores con 1+ Hits (Top 4)", type="primary", use_container_width=True):
            with st.spinner("Calculando probabilidades de dar hits..."):
                ...
                if resultados_hits:
                    ...
                    st.warning("No se encontraron bateadores con datos suficientes para hoy en la LVBP.")
        ...
        if clave_hits in st.session_state and st.session_state[clave_hits] is not None:
            ...
            st.download_button(
                ...
                data=excel_hits,
                file_name=f"caza_hits_lvbp_{st.session_state.fecha_hoy}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            )
    ...
    with tab5:
        ...
        if st.button("🔍 Ejecutar Auditoría Premium", type="primary", use_container_width=True):
            ...
            for idx, fecha_str in enumerate(fechas_auditar):
                estado.write(f"⏳ Procesando {fecha_str} (Analizando pitcheo de élite en LVBP)...")

                # 9. ADAPTACIÓN A SPORTID=17
                juegos_dia = statsapi.schedule(date=fecha_str, sportId=17)
        ...
        if "auditoria_7dias" in st.session_state and st.session_state.auditoria_7dias:
            ...
            if resultados:
                ...
                st.download_button(
                    ...
                    file_name="auditoria_premium_lvbp_7dias.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                )
    ...
    with tab6:
        ...
        # 11. ADAPTACIÓN A SPORTID=17
        juegos_dia_lupa = statsapi.schedule(date=st.session_state.fecha_hoy, sportId=17)
        # Filtramos también por la whitelist para no mostrar juegos de otras ligas caribeñas en el selector
        juegos_validos = [
            j for j in juegos_dia_lupa 
            if j.get('status') not in ['Postponed', 'Cancelled'] 
            and j['home_name'] in LVBP_TEAM_WHITELIST
        ]
        
        if not juegos_validos:
            st.info("No hay juegos LVBP programados para esta fecha.")
        else:
